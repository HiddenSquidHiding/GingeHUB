-- WoodzHUB_Minimal_Farm_MinimalFull.lua
-- Script with minimal full Farm module to debug 'attempt to call a nil value' error
...
-- Utils.lua (With minimal functions for Farm)
        ...
        local function classify(node)
            if node:IsA("Model") and not Players:GetPlayerFromCharacter(node) then
                ...
                if h and h.Health > 0 then
                    ...
                    debugPrint("Found enemy: " .. node.Name)
                end
            end
    ...
    local function loop(ctx, ui, deps)
        ...
        while running do
            ...
            if #enemies == 0 then
                ...
                Utils.notify("WoodzHUB", "Found " .. #enemies .. " enemies", 3)
            end
            task.wait(1) -- Minimal loop, no teleportation
        end
